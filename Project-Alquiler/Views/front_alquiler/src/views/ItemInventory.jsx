...
const ItemInventory = () => {
  ...
  const handleGetArticulo = async () => {
    try {
      ...
      const newOptions = data.map((element) => ({
        ...
        label: element.Descripcion, //lo que se ve en el selector
      }));
    ...
    if (window.prompt("Ingrese la credencial de autorizacion", 0) == "202312") {
      if (window.confirm("¿Estás seguro de que quieres eliminar?")) {
        try {
          ...
          setForm((prev) =>
            prev.filter((info) => info.IdInventarioArticulo != id)
          );
          setDeleted(true);
          if (selected && selected.IdInventarioArticulo == id) {
            setSelected(null);
            setNews({
              IdInventarioArticulo: "",
              Cantidad: "",
  ...
  const handleEdit = async (news) => {
    setSelected(news);
    setNews({
      IdInventarioArticulo: news.IdInventarioArticulo,
  ...
  const handleUpdate = async () => {
    const response = await fetch(
      `${URL}${PORT}/${form}/${selected.IdInventarioArticulo}`,
      {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
    ...
    setForm((prev) =>
      prev.map((estado) =>
        estado.IdInventarioArticulo === data.IdInventarioArticulo
          ? data
          : estado
      )
    );
  ...
  return (
    <>
      <div className="container mt-4">
        <div className="row">
          <div className="col">
            <TitleCataComponente title="inventario de articulos" size="h6" />
        ...
        <div className="row">
          ...
          <div className="col-12 col-sm-12 col-md-12 col-lg-8 col-xl-8">
            <TabletCataComponente
              ...
              idField={"IdInventarioArticulo"}
              Fields={["Cantidad", "IdArticulo"]}
            />
