...
export const RolForm = () => {
  ...
  const [news, setNew] = useState({ description_rol: "" });
  ...
  const form = 'rols';
  const title = useRef("Administrador de roles") //no actualiza la pagina en cambio el state si 
  ...
  const handleSubmit = async (e) => {
    ...
    if (selected) {
      if (window.confirm("¿Estás seguro de que quieres actualizar este rol?")) {
        try {
          // Enviar solicitud para actualizar un rol existente
          const response = await fetch(`${apiUrl}${port}/${form}/${selected.id_rol}`, {
            method: "PUT",
            headers: {
          ...
          const data = await response.json();
          setForm((prev) =>
            prev.map((rol) => (rol.id_rol == data.id_rol ? data : rol))
          );
          setSelected(null);
          setNew({ description_rol: "" });
        } catch (error) {
      ...
      try {
        ...
        data.error == "Ocurrió un error en el servidor" ?  alert("ya existe") : setForm((prev) => [...prev, data]); setNew({ description_rol: "" });
      } catch (error) {
  ...
  const handleMultipleDelete = async (ids) => {
    if (window.confirm("¿Estás seguro de que quieres eliminar los roles seleccionados?")) {
      try {
  ...
  const handleEdit = (info) => {
    ...
    setSelected(info);
    setNew({ id_rol: info.id_rol, description_rol: info.description_rol });
  };
      ...
      <div className="row">
        ...
        <div className="col-12 col-sm-12 col-md-12 col-lg-6 col-xl-6">
          <MyTableComponent
            ...
            idField={"id_rol"}
            Fields={["description_rol", "createdAt", "updatedAt"]}
          />
