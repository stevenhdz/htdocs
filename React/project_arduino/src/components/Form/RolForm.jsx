...
export const RolForm = () => {
      ...
      if (window.confirm("¿Estás seguro de que quieres actualizar este rol?")) {
        try {
          ...
          const response = await fetch(
            `${apiUrl}${port}/${form}/${selected.id_rol}`,
            {
              method: "PUT",
              headers: {
          ...
          data.error == "Ocurrió un error en el servidor"
            ? alert("Ya existe otro rol llamado igual.")
            : alert(data.message);
          setForm((prev) =>
  ...
  const handleMultipleDelete = async (ids) => {
    if (
      window.confirm(
        "¿Estás seguro de que quieres eliminar los roles seleccionados?"
      )
    ) {
      try {
  ...
  const handleDelete = async (id) => {
    if (id > 2) {
      ...
      alert("No esta permitido para los roles por defecto.");
    }
  };
