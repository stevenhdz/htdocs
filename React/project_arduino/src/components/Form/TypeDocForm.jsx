...
export const TypeDocForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (selected) {
      if (window.confirm("¿Estás seguro de que quieres actualizar este tipo documento?")) {
        try {
          // Enviar solicitud para actualizar un rol existente
          const response = await fetch(`${apiUrl}${port}/${form}/${selected.id_typedoc}`, {
            method: "PUT",
            headers: {
  ...
  const handleMultipleDelete = async (ids) => {
    if (window.confirm("¿Estás seguro de que quieres eliminar los tipos de documentos seleccionados?")) {
    ...
    data.error == "Ocurrió un error en el servidor" ?  alert("ya existe") : setForm((prev) => [...prev, data]); setNew({ description_type_doc: "" });
  }
  }
  ...
  const handleEdit = (info) => {
    ...
    setSelected(info);
    setNew({ id_typedoc: info.id_typedoc, description_type_doc: info.description_type_doc });
  };
  ...
  return (
    ...
    <div className="container mt-4">
      <div className="row">
        <div className="col">
          <TitleComponent title="Administrador de tipos de documentos" size="h1" />
        </div>
      </div>
      <div className="row">
