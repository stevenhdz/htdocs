...
Class Consultas
{
	...
	public function ventasultimos_12meses()
	{
		...
		$sql="SELECT DATE_FORMAT(fecha_hora, '%c/%Y') AS fecha, COUNT(*) AS total_venta,SUM(total_venta) as total FROM venta GROUP BY DATE_FORMAT(fecha_hora, '%c/%Y') ORDER BY MIN(fecha_hora) DESC limit 0,10";
		return ejecutarConsulta($sql);
	} 
	

	public function casosAtendidos()
	{
		$sql="SELECT IFNULL(SUM(valortotal),0) as valortotal FROM soporte";
		return ejecutarConsulta($sql);
	}

	public function casosAtendidos2()
	{
		$sql="SELECT DATE_FORMAT(fechaentrada, '%c/%Y') AS fecha, COUNT(*) AS valortotal,SUM(valortotal) as total FROM soporte GROUP BY DATE_FORMAT(fechaentrada, '%c/%Y') ORDER BY MIN(fechaentrada) DESC limit 0,10";
		return ejecutarConsulta($sql);
	}


	public function cantidadPCAtendidos()
	{
		$sql="SELECT IFNULL(SUM(cantidadequipos),0) as cantidadequipos FROM soporte";
		return ejecutarConsulta($sql);
	}

	public function cantidadPCAtendidos2()
	{
		$sql="SELECT DATE_FORMAT(fechaentrada, '%c/%Y') AS fecha, COUNT(*) AS valortotal,SUM(valortotal) as total FROM soporte GROUP BY DATE_FORMAT(fechaentrada, '%c/%Y') ORDER BY MIN(fechaentrada) DESC limit 0,10";
		return ejecutarConsulta($sql);
	}
